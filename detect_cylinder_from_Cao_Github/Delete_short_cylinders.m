...
for i=1:size(MergedCylinders,2)
     ...
     % if angleInDegrees > 2.5  ptCloudMov = pcread("BigTube_AllScanedPC.pcd");
     % if angleInDegrees > 2.5  ptCloudMov = pcread("Tube1_scaned.ply");
     % if angleInDegrees > 4    ptCloudMov = pcread("Tube2_scaned.ply");
     % if angleInDegrees > 5    ptCloudMov = pcread("Tube3_scaned.ply");
